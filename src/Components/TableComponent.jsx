...
const TableComponent = ({ title, data, columns }) => {
  ...
  const toggleColumn = (columnIndex) => {
    setVisibleColumns(prev =>
      prev.map((col, index) => (index === columnIndex ? { ...col, visible: !col.visible } : col))
    );
  };

  return (
    <div className="mb-8 bg-emerald-100">
            ...
            {visibleColumns.map((col, index) => (
              col.visible && (
                <th
                  ...
                  className="p-2 border border-gray-300 cursor-pointer md:table-cell block"
                >
                  {col.label}
                </th>
              )
          ...
          {data.map((row, rowIndex) => (
            <React.Fragment key={rowIndex}>
              ...
              >
                {visibleColumns.map((col, index) => (
                  col.visible && (
                    <td key={index} className="p-2 border border-gray-300 md:table-cell block md:table-cell">
                      {row[col.key]}
                    </td>
                  )
                ))}
              ...
              {expandedRows[rowIndex] && (
                <tr className="bg-gray-100 md:table-row block md:table-row">
                  <td colSpan={visibleColumns.length} className="p-2 border border-gray-300 md:table-cell block md:table-cell">
                    <div className="p-2">
